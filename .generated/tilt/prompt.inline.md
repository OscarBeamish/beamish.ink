...
**`src/beamish/effects/tilt/core.ts`**
...
export type TiltOptions = BaseOptions & {
 ...
 */
export const tiltDefaults: TiltOptions = {
  maxTilt: 9,
  perspective: 900,
  scale: 1.02,
  sheen: 0.5,
...
class TiltSurface implements Surface<TiltOptions> {
  ...
  render(_t: number, opts: TiltOptions, pointer: Pointer): void {
 ...
 *
 * ```ts
 * const tilt = createTilt(document.querySelector('.card')!)
 * tilt.start()
 * // …later
 * tilt.destroy()
 * ```
 ...
 */
export function createTilt(el: HTMLElement, opts: Partial<TiltOptions> = {}): EffectHandle {
  return mount<TiltOptions>(el, opts, {
    defaults: tiltDefaults,
    kind: 'dom',
    create: () => new TiltSurface()
  })
}

export default createTilt
```

You are adding **PointerRippleGrid** from Beamish to this project.

> A field of matte forms on paper, rippling around the cursor. Pointer · effect · MIT.
> https://beamish.ink/effects/pointer-ripple-grid
...
**`src/beamish/effects/pointer-ripple-grid/core.ts`**
...
export type PointerRippleGridOptions = BaseOptions & {
 ...
 */
export const pointerRippleGridDefaults: PointerRippleGridOptions = {
  paper: '#fbfaf4',
  stone: '#f2efe6',
  accent: '#c44400',
  count: 18,
...
class PointerRippleGridSurface implements Surface<PointerRippleGridOptions> {
   ...
   */
  private buildField(opts: PointerRippleGridOptions): void {
  ...
  render(t: number, opts: PointerRippleGridOptions, pointer: Pointer): void {
 ...
 *
 * ```ts
 * const swell = createPointerRippleGrid(document.querySelector('#field')!)
 * swell.start()
 * // …later
 * swell.destroy()
 * ```
 */
export function createPointerRippleGrid(el: HTMLElement, opts: Partial<PointerRippleGridOptions> = {}): EffectHandle {
  return mount<PointerRippleGridOptions>(el, opts, {
    defaults: pointerRippleGridDefaults,
    create: () => new PointerRippleGridSurface()
  })
}

export default createPointerRippleGrid
```
...
<script type="module">
  import { createPointerRippleGrid } from './beamish/effects/pointer-ripple-grid/core.js'

  const swell = createPointerRippleGrid(document.querySelector('#field'), { count: 18 })
  swell.start()
</script>
```
...
import { createPointerRippleGrid } from '@/beamish/effects/pointer-ripple-grid/core'
...
export function Field() {
  ...
  useEffect(() => {
    ...
    const swell = createPointerRippleGrid(host.current, { count: 18, accent: '#c44400' })
    swell.start()
...
onMounted(() => {
  ...
  swell = createPointerRippleGrid(host.value, { count: 18 })
  swell.start()
})
...
`client:visible`, or call `createPointerRippleGrid` from a plain `<script>` in the page.
...
None of this runs on the server. `createPointerRippleGrid` touches `document` and

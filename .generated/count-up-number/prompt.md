You are adding **CountUpNumber** from Beamish to this project.
...
| `src/beamish/effects/count-up-number/core.ts` | https://raw.githubusercontent.com/OscarBeamish/beamish.ink/{{PIN}}/effects/count-up-number/core.ts |
...
<script type="module">
  import { createCountUpNumber } from './beamish/effects/count-up-number/core.js'
...
import { createCountUpNumber } from '@/beamish/effects/count-up-number/core'
...
export function Stat({ value }: { value: string }) {
  ...
  useEffect(() => {
    ...
    const tally = createCountUpNumber(host.current, { duration: 1800 })
    tally.start()
...
onMounted(() => {
  ...
  tally = createCountUpNumber(host.value)
  tally.start()
})
...
it to be. Call `createCountUpNumber` from `useEffect`, `onMounted`, or a `client:*`
island.
...
- https://raw.githubusercontent.com/OscarBeamish/beamish.ink/{{PIN}}/effects/count-up-number/adapters/react.tsx
- https://raw.githubusercontent.com/OscarBeamish/beamish.ink/{{PIN}}/effects/count-up-number/adapters/vue.ts

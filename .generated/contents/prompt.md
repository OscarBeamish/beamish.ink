...
| --- | --- | --- | --- | --- |
| `label` | string | `Index` | — | The trigger's text, and the dialog's accessible name. Keep it a noun — it is read out as the name of the whole overlay. |
| `closeLabel` | string | `Close` | — | Text on the close control. There is a visible one as well as Escape, because Escape is not discoverable. |

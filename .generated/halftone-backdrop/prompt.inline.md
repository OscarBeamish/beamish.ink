...
**`src/beamish/effects/halftone-backdrop/core.ts`**
 ...
 *
 * The shader source is generated from shaders/halftone-backdrop.frag and
 * shaders/halftone-backdrop.vert. Edit those, then run `pnpm generate`. The markers are
 * load-bearing.
 */

import { mount, type BaseOptions, type EffectHandle, type Surface } from '../../shared/runtime'

export type HalftoneBackdropOptions = BaseOptions & {
...
// beamish:shader-begin shaders/halftone-backdrop.vert
...
// beamish:shader-begin shaders/halftone-backdrop.frag
...
class HalftoneBackdropSurface implements Surface<HalftoneBackdropOptions> {
  ...
  render(t: number, opts: HalftoneBackdropOptions): void {
    const gl = this.gl
 ...
 *
 * ```ts
 * const overprint = createHalftoneBackdrop(document.querySelector('#bg')!)
 * overprint.start()
 * // …later
 * overprint.destroy()
 * ```
 */
export function createHalftoneBackdrop(
  el: HTMLElement,
  opts: Partial<HalftoneBackdropOptions> = {}
): EffectHandle {
  return mount<HalftoneBackdropOptions>(el, opts, {
    defaults: halftoneBackdropDefaults,
    create: () => new HalftoneBackdropSurface()
  })
}

export default createHalftoneBackdrop
```

...
import { createHalftoneBackdrop, type HalftoneBackdropOptions } from '../core'
import type { EffectHandle } from '../../../shared/runtime'

export type HalftoneBackdropProps = Partial<HalftoneBackdropOptions> & {
  className?: string
  /** Set false to mount without starting. Useful behind your own pause control. */
  autoStart?: boolean
}

export function Overprint({ className, autoStart = true, ...options }: HalftoneBackdropProps) {
  ...
  useEffect(() => {
    ...
    const effect = createHalftoneBackdrop(host.current, options)
